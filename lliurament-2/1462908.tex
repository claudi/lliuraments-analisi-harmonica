...
% https://math.stackexchange.com/a/2046706/457514
...
\[
    ...
    \abs{\widehat{f}(\xi)}\abs{e^{2\uppi\iu h\xi}-1} \diff\xi.
\]
Com que~\(\abs{e^{2\uppi\iu h\xi}-1}\leq2\) tenim que
\begin{align*}
    I_{2} &\leq \int_{\abs{\xi}\geq\abs{h}^{-1}}
                \abs{\widehat{f}(\xi)}\diff\xi \\
          &\leq \int_{\abs{\xi}\geq\abs{h}^{-1}}
                \frac{1}{\abs{\xi}^{1+\alpha}} \diff\xi \\
          &\leq h^{\alpha},
\end{align*}
i per tant~\(I_{2} = \mathrm{O}(h^{\alpha})\).
